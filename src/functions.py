...
# Рислование графиков автокорреляции и добавление графиков автокорреляции.
...
def print_corr(stocks, stock_key_1, stock_key_2):
    ...
    print('Корреляция между ' + stock_1.name + '(' + stock_1.key + ')' + ' и ' + stock_2.name + '(' +
          stock_2.key + ')' + ' = ' + str(round(corr_data.corr()[0][1], 2)))
...
def print_corr_profit_volume(stocks, stock_key):
    ...
    corr_data[1] = corr_data_2
    print('Корреляция между доходностью и объёмом для ' + stock.name + '(' + stock.key + ') = '
          + str(round(corr_data.corr()[0][1], 2)))
